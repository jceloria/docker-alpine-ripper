...
RUN cd /tmp/build && \
    ...
    apk --no-cache add curl cdparanoia ddrescue flac libdiscid openrc py3-udev py3-pillow util-linux && \
    apk add --no-cache -X http://dl-cdn.alpinelinux.org/alpine/edge/testing cdrdao && \
    ./whipper/install-whipper.sh && ./ccextractor/install-ccextractor.sh && udevadm hwdb --update && \
